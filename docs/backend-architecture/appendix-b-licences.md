...
| `Dapper` | Apache 2.0 | Read-side data access ([§6.5](06-cqrs.md)), the raw writes `IUnitOfWork.ExecuteRawAsync` makes on the command's own transaction (§6.3), and the outbox dispatcher's claim, complete and fail statements in `Common.Infrastructure` ([§9.4](09-messaging.md)) — which are neither, running on a connection of their own outside any command. All three named, because a register row saying "read-side" reads as an audit of where the package may appear, and a use this row does not list would then look unregistered |
...
| MassTransit **8.x** (`MassTransit`, `MassTransit.RabbitMQ`) | Apache 2.0 | Messaging. Two identities on the health-check row's terms: the transport package is what each `*.Infrastructure` references, and the core package is referenced directly twice — by the harness smoke ([§12.5](12-test-strategy.md)), because the in-memory harness is core API and a test project that uses no transport must not claim one, and by `Common.Infrastructure` since PR-14, for the `IPublishEndpoint` the outbox dispatcher publishes the Broker lane through ([§9.4](09-messaging.md)). That second use is the first common code to name a MassTransit type, and it is named here on the `Dapper` row's standard: a role that reads as an audit of where a package may appear has to list every place it does |
