...
| `IIdempotentCommand` | §8.5 | Opts a command into `IdempotencyBehavior`. Two members, not one: `CommandId`, and a `static abstract OperationName` the compiler forces every implementor to supply — the key's operation segment is declared so that a rename cannot silently change a live key |
