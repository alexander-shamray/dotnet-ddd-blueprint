...
| `OrderCancelled` | Envelope, `OrderId`, `CustomerId`, `Reason` — a `CancelReasons` code. When the saga caused the cancellation it is the code the saga sent on `CancelOrder`, so the fact is reported with the reason that produced it; §11.4's endpoint sends whatever the caller asked for, which is **any** of the five codes `CancellationReasons` parses and not `customer_request` alone. **So `Reason` does not identify the origin**, and this entry used to say it did — the event carries no field that does. **The saga consumes it as well as causing it** (§3.2, §9.6): the endpoint is the only way a customer's cancellation reaches the workflow, and a copy arriving after the saga finalised is discarded rather than faulted |
