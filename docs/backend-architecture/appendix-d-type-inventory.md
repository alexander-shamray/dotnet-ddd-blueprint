...
| `OrderSummaryProjection` | §6.6 | Writes `ordering.OrderSummaries`, from Ordering's five lifecycle events on the local lane and Catalog's `ProductPublished` from the broker. **Two watermarks, not one**: `UpdatedAt` versions the lifecycle sequence and `ProductsUpdatedAt` the copied-fields one, because the two are stamped from two services' clocks and a column advanced by both discards whichever stream is behind — a status lost for ever in one direction, a rename in the other |
