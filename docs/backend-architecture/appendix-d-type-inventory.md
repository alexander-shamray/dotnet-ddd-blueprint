...
| `ClientCredentialsHandler` | §11.5 | `DelegatingHandler`; attaches the M2M bearer token to the BFF's outbound calls. Registered **after** the resilience handler so it sits inside it — a retry then re-attaches a fresh token, which is the one case a retry after a 401 can help |
| `PricingService`, `ValidationInterceptor` | §9.7 | Catalog's server half of the pricing hop, and the interceptor turning a `ValidationException` into `InvalidArgument`. Without the second, a malformed request reaches the caller as `Unknown` — which the BFF's pipeline treats as transient and retries |
...
| `UpstreamExceptionHandler` | §9.7 | `IExceptionHandler` in `Web.Bff`; `RpcException` to a status a caller can act on — `InvalidArgument` to 400, `Unavailable`/`DeadlineExceeded` to 503, everything else left **unhandled** so it stays a logged 500 rather than a quietly reclassified one |
