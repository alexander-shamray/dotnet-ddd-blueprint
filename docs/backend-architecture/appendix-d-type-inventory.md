...
| `InboxFilter<T>` | §9.5 | `IFilter<ConsumeContext<T>>` over `DbContext` — common, reaching the entity through `Set<InboxMessage>()`. Each service registers `AddScoped<DbContext>(sp => sp.GetRequiredService<XDbContext>())`, and the **delegate** is what keeps the inbox row in the handler's transaction |
