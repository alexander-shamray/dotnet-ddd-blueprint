...
| `IntegrationCollection` (xUnit) | The `[CollectionDefinition]` sharing one `ServiceFixture` across test classes — declared once **per test assembly** (§12.4). Named for what it groups, and deliberately not `ServiceCollection`: that name is taken by `Microsoft.Extensions.DependencyInjection`, and the local type would win in every test file that also builds a provider. It also carries `[Trait("Category", "Integration")]`, which xUnit v3 applies to every test in the collection — so the declaration that grants a test its containers is the same one that puts it in the slow half, and there is no per-class attribute to forget. `Web.Bff.Tests`'s `KeycloakCollection` is the same type by another name, over the one fixture that suite shares |
