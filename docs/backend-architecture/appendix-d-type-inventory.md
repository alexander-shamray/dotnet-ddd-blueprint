...
| `IdempotencyBehavior<,>` | §8.5 | `IPipelineBehavior<,>` over `IIdempotencyStore` and `ICurrentUser` — the second scopes the claim to a subject. Constrained `TResult : Result`, which the container enforces by omission |
...
| `StockReservationExpired`, `PaymentAuthorisationExpired`, `DespatchExpired`, `StockReleaseExpired` | §9.6 | The four scheduled messages, one per wait state. **Not contracts and deliberately not in `Common.Contracts`**: they appear in no column of §3.2, the saga sends them to itself, and putting them where §4.3's one crossing assembly lives would advertise four messages a peer could legitimately publish — which is how a saga acquires a way to be driven from outside. One record per wait rather than one carrying a discriminator, because MassTransit correlates a schedule by message **type**: a single type makes all four schedules one subscription, and a stale expiry could not then be told apart from a live one. **Not "so `Unschedule` knows which token to cancel"** — on ADR-021's scheduler `Unschedule` cancels nothing, so what discriminates an expiry is its type against the state it arrives in |
...
| `FlagOrderForReviewHandler` | §9.6 | `ICommandHandler<FlagOrderForReviewCommand, Result>`, printed in full by the chapter — which is why it is here rather than in D.5 beside the two saga handlers that are not. Writes the `OrderReviews` row through `IUnitOfWork.ExecuteRawAsync` and loads no aggregate: nothing about the order has changed, only the process stalled |
| `OrderReview` | §9.6 | A row of `ordering.OrderReviews` — mapped by EF so the migration owns the schema, written by Dapper through `IUnitOfWork.ExecuteRawAsync`, and given no `DbSet` so nothing acquires a second write path around the conditional insert's range lock. Deliberately not a domain type: an escalation changes nothing about the order |
...
| `StartHarnessAsync` | Test helper returning a started MassTransit harness, and where a saga suite states both harness bounds — which a **positive** assertion waits under, since §12.5's negatives read the record on a cancelled token and wait for nothing |
