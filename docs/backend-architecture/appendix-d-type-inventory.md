...
| `OrderSummaryProjection` | §6.6 | Writes `ordering.OrderSummaries`, from Ordering's five lifecycle events on the local lane and Catalog's `ProductPublished` from the broker. **Two watermarks, not one**: `UpdatedAt` versions the lifecycle sequence and `ProductsUpdatedAt` the copied-fields one, because the two are stamped from two services' clocks and a column advanced by both discards whichever stream is behind — a status lost for ever in one direction, a rename in the other. **`ProductsUpdatedAt` is still one watermark for many streams**: an order has as many `ProductPublished` sequences as it has products, so a delayed event for one product, handled after a newer one for another, is discarded — and since the summary's names are never filled at insert ([#121](https://github.com/alexander-shamray/dotnet-ddd-blueprint/issues/121)), that is a name never set rather than a rename missed. Both are §6.6's, stated there and repeated here so this row cannot be copied as a finished design |
...
| `GetOrderSummariesQuery`, `GetOrderSummariesHandler`, `OrderSummaryDto`, `SummaryProduct` | The worked query slice — shown at level 1 (§6.5) and rewritten in place at level 2 (§6.6). Carries `Cursor` and `Limit` only: the `CustomerId` in the `WHERE` clause is bound from `ICurrentUser` at both levels (§11.4's subject rule). The projection feeding it is `OrderSummaryProjection`, defined in D.4. `SummaryProduct` is **also the element the projection serialises** into the `Products` column, and its three JSON names are pinned with `[JsonPropertyName]` — the `JSON_MODIFY` paths in the `ProductPublished` handler spell them a third time, and nothing but the attribute holds a C# rename to that literal |
