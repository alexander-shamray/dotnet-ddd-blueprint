...
| `CancelOrderMapper`, `ConfirmOrderMapper`, `MarkOrderShippedMapper`, `FlagOrderForReviewMapper` | §9.4, §9.6 | `ICommandMessageMapper<,>`, one per command in §3.2's Accepts column. Each does the two things `CommandConsumer` deliberately does not — parse the wire vocabulary, and declare `CommandOrigin.System` — and each throws `ContractMappingException` rather than letting a `DomainException` reach the handler, so a payload this service cannot read goes to the error queue on the first attempt instead of being acked as a domain rejection |
