...
| `NoTransformResponseCompressionProvider` | [ADR-020](appendix-a-adrs.md#adr-020--the-edge-compresses-over-tls-and-says-so) | `Gateway.Api`; a `ResponseCompressionProvider` subclass adding one case in front of `ShouldCompressResponse` — a response carrying `Cache-Control: no-transform` is not compressed. RFC 9111 §5.2.2.6 requires that of an intermediary and ASP.NET Core does not implement it, so this is conformance rather than policy. Registered with `Replace`, because `AddResponseCompression` uses `TryAddSingleton` and ordering would otherwise decide it silently |
