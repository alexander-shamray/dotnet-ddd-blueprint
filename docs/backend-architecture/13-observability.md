...
| Orders awaiting review | any row in `ordering.OrderReviews` older than 1 h | A saga escalated work it has no contract to do (§9.6) — a wait it could not compensate, or an authorisation that landed once the order was already being cancelled. **The second is not necessarily a customer cancelling**, and the runbook used to say it was: compensation begins on a cancellation, a decline or the fifteen-minute payment timeout alike, so a slow PSP raises that row with nobody having cancelled anything. **Whether the saga has finalised is a property of the reason and the state, not of the row**: every reason but one is raised on the way out, while `payment_authorised_during_compensation` from `Compensating` is raised mid-wait and can sit beside a live instance for the ten-minute release window. **That is the only case the saga-age alert can reach, and it will not usually reach it either**: both thresholds are an hour, and `Compensating` normally ends inside those ten minutes, so the instance is gone well before this row is old enough to page. The two coincide only when `StockReleased` and `ReleaseTimeout` have both failed — which is why this row exists rather than being folded into the one above | `order-review.md` |
