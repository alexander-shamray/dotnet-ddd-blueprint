...
| Orders awaiting review | any row in `ordering.OrderReviews` older than 1 h | A saga escalated work it has no contract to do (§9.6) — a wait it could not compensate, or a cancellation with money already authorised. It has already finalised, so the saga-age alert above will *not* catch this | `order-review.md` |
