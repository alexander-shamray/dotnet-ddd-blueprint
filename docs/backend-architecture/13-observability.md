...
| Migration job failed | Helm `pre-install,pre-upgrade` hook non-zero, or a release stuck pending | The deploy stopped before any pod rolled ([§7.4](07-persistence.md)); the previous version is still serving, which is why nothing else fires | `migration-failure.md` |
