...
and `dotnet test` runs 930 tests — so the build rules and the drift rules below
...
dotnet test  Platform.slnx --filter "Category!=Integration"   # 738 of 930, no daemon
```
...
the 930 and starts no container — measured with `docker events`, not inferred —
and `Category=Integration` is the other 192, needing the daemon exactly as
before.
...
(18), unit (720) and integration (192), which is the 738 above split at the
