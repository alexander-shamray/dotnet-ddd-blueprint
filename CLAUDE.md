...
and `dotnet test` runs 912 tests — so the build rules and the drift rules below
...
dotnet test  Platform.slnx --filter "Category!=Integration"   # 722 of 912, no daemon
```
...
category runs a smaller suite and says which. `Category!=Integration` is 722 of
the 912 and starts no container — measured with `docker events`, not inferred —
...
(18), unit (704) and integration (190), which is the 722 above split at the
