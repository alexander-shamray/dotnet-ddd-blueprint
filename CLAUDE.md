...
and `dotnet test` runs 901 tests — so the build rules and the drift rules below
...
dotnet test  Platform.slnx --filter "Category!=Integration"   # 711 of 901, no daemon
```
...
category runs a smaller suite and says which. `Category!=Integration` is 711 of
the 901 and starts no container — measured with `docker events`, not inferred —
...
(18), unit (693) and integration (190), which is the 711 above split at the
