...
and `dotnet test` runs 1,014 tests — so the build rules and the drift rules
...
dotnet test  Platform.slnx --filter "Category!=Integration"   # 818 of 1,014, no daemon
```
...
category runs a smaller suite and says which. `Category!=Integration` is 818 of
the 1,014 and starts no container — measured with `docker events`, not
...
(18), unit (800) and integration (196), which is the 818 above split at the
