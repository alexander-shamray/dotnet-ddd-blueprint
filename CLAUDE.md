...
and `dotnet test` runs 885 tests — so the build rules and the drift rules below
...
dotnet test  Platform.slnx --filter "Category!=Integration"   # 697 of 885, no daemon
```
...
category runs a smaller suite and says which. `Category!=Integration` is 697 of
the 885 and starts no container — measured with `docker events`, not inferred —
...
(18), unit (679) and integration (188), which is the 697 above split at the
