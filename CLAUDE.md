...
and `dotnet test` runs 895 tests — so the build rules and the drift rules below
...
dotnet test  Platform.slnx --filter "Category!=Integration"   # 707 of 895, no daemon
```
...
category runs a smaller suite and says which. `Category!=Integration` is 707 of
the 895 and starts no container — measured with `docker events`, not inferred —
...
(18), unit (689) and integration (188), which is the 707 above split at the
