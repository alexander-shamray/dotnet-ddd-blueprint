...
and `dotnet test` runs 1,040 tests — so the build rules and the drift rules
...
dotnet test  Platform.slnx --filter "Category!=Integration"   # 839 of 1,040, no daemon
```
...
the 1,040 and starts no container — measured with `docker events`, not
inferred — and `Category=Integration` is the other 201, needing the daemon
exactly as before.
...
(18), unit (821) and integration (201), which is the 839 above split at the
