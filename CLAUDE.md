...
and `dotnet test` runs 841 tests — so the build rules and the drift rules below
...
dotnet test  Platform.slnx --filter "Category!=Integration"   # 654 of 841, no daemon
```
...
category runs a smaller suite and says which. `Category!=Integration` is 654 of
the 841 and starts no container — measured with `docker events`, not inferred —
...
(18), unit (636) and integration (187), which is the 654 above split at the
