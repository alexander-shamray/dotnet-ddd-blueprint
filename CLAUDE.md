...
and `dotnet test` runs 935 tests — so the build rules and the drift rules below
...
dotnet test  Platform.slnx --filter "Category!=Integration"   # 739 of 935, no daemon
```
...
the 935 and starts no container — measured with `docker events`, not inferred —
...
(18), unit (721) and integration (196), which is the 739 above split at the
