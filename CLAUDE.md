...
and `dotnet test` runs 1,020 tests — so the build rules and the drift rules
...
dotnet test  Platform.slnx --filter "Category!=Integration"   # 824 of 1,020, no daemon
```
...
category runs a smaller suite and says which. `Category!=Integration` is 824 of
the 1,020 and starts no container — measured with `docker events`, not
...
(18), unit (806) and integration (196), which is the 824 above split at the
