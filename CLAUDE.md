...
and `dotnet test` runs 881 tests — so the build rules and the drift rules below
...
dotnet test  Platform.slnx --filter "Category!=Integration"   # 693 of 881, no daemon
```
...
category runs a smaller suite and says which. `Category!=Integration` is 693 of
the 881 and starts no container — measured with `docker events`, not inferred —
...
(18), unit (675) and integration (188), which is the 693 above split at the
