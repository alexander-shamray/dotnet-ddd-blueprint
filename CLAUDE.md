...
and `dotnet test` runs 903 tests — so the build rules and the drift rules below
...
dotnet test  Platform.slnx --filter "Category!=Integration"   # 713 of 903, no daemon
```
...
category runs a smaller suite and says which. `Category!=Integration` is 713 of
the 903 and starts no container — measured with `docker events`, not inferred —
...
(18), unit (695) and integration (190), which is the 713 above split at the
