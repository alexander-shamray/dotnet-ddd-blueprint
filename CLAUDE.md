...
and `dotnet test` runs 850 tests — so the build rules and the drift rules below
...
dotnet test  Platform.slnx --filter "Category!=Integration"   # 663 of 850, no daemon
```
...
category runs a smaller suite and says which. `Category!=Integration` is 663 of
the 850 and starts no container — measured with `docker events`, not inferred —
...
(18), unit (645) and integration (187), which is the 663 above split at the
