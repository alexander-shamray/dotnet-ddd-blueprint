...
and `dotnet test` runs 899 tests — so the build rules and the drift rules below
...
dotnet test  Platform.slnx --filter "Category!=Integration"   # 711 of 899, no daemon
```
...
the 899 and starts no container — measured with `docker events`, not inferred —
...
(18), unit (693) and integration (188), which is the 711 above split at the
