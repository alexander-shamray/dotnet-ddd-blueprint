...
and `dotnet test` runs 910 tests — so the build rules and the drift rules below
...
dotnet test  Platform.slnx --filter "Category!=Integration"   # 720 of 910, no daemon
```
...
category runs a smaller suite and says which. `Category!=Integration` is 720 of
the 910 and starts no container — measured with `docker events`, not inferred —
...
(18), unit (702) and integration (190), which is the 720 above split at the
