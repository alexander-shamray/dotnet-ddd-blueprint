...
and `dotnet test` runs 914 tests — so the build rules and the drift rules below
...
dotnet test  Platform.slnx --filter "Category!=Integration"   # 724 of 914, no daemon
```
...
category runs a smaller suite and says which. `Category!=Integration` is 724 of
the 914 and starts no container — measured with `docker events`, not inferred —
...
(18), unit (706) and integration (190), which is the 724 above split at the
