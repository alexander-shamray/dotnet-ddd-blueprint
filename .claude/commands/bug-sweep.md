...
- **Not already tracked.** Before filing, enumerate the **whole** issue set —
  ...
  name the same lines. An open issue whose author is the repository **owner**,
  or which carries a maintainer-applied label, blocks a re-file — as does a
  `wontfix` or an accepted-risk record meeting the same test. **An issue
